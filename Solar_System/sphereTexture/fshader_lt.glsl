...
main()
{
    ...
    //vec4 specular = SpecularLight * kS * texel * pow( max(dot(N, R), 0.0), Shininess );
    vec4 specular = SpecularLight * ks * texel * pow( max(dot(R, E), 0.0), Shininess ); //- use instead
    //vec4 specular = SpecularLight * ks * texel * pow( max(dot(reflect(-L,N ), 0.0), Shininess ); //- use instead
